...
function log_filtering(){
	...
	do
		...
		check_nfs $i
		check_ntp $i
		RHELVER=`cat ./${i}/etc/redhat-release | awk '{print $7}' | awk -F "." '{print $1}'`
		echo -ne "Host Name : ${i}           \n"
		if [ $RHELVER -eq 7 ]
		then
			check_daemon_7
